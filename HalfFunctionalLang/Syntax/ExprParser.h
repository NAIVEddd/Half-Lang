...
Parser<Def_Func> GetDeffuncParser()
{
	return [](ParserInput s) -> ParserResult<Def_Func>
		{
			auto space = Spaces();
			...
			Converter2<std::string, std::string, Def_Func::TypeField> tofield =
				[](std::string a, std::string b)
				{
					Def_Func::TypeField field;
					field.name = a;
					field.type = b;
					return field;
				};
			auto ppair = Pipe2(pname, pname, tofield);
			auto pairs = Between(keyopen, keyclose, OneOrZero(SepBy(ppair, keysep)));

			auto rname = (keyfunc > pname)(s);
			if (!rname)
			{
				return std::nullopt;
			}
			...
			auto parsepairs = (pairs < keyassign);
			auto rpairs = parsepairs(s);
			if (!rpairs)
			{
				return std::nullopt;
			}
			...
			auto p = rpairs.value().first;
			return std::make_pair(Def_Func(rname.value().first, p, "", rbody.value().first), s);
		};
}

ParserResult<Def_Func> pdeffunc(ParserInput s)
{
	static auto parser = GetDeffuncParser();
	return parser(s);
}
	...
	return [](ParserInput s) ->ParserResult<Half_Expr>
		{
			...
			auto fundefparser = PipeExpr(pdeffunc);
